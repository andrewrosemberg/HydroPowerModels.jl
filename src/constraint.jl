...
function constraint_hydro_generation(sp, data::Dict, pm::GenericPowerModel)
    @constraint(sp, turbine_energy[i=1:data["hydro"]["nHyd"]], var(pm, 0, 1, :pg)[data["hydro"]["Hydrogenerators"][i]["i_grid"]]*data["powersystem"]["baseMVA"] == 0.0036*sp[:outflow][i]*data["hydro"]["Hydrogenerators"][i]["production_factor"]
    )
    return nothing
end