...
function variable_deficit(sp, data::Dict, pm::AbstractPowerModel)
    PowerModels.var(pm)[:deficit] = @variable(sp, deficit[i in collect(1:length(PowerModels.sol(pm, 0, :bus)))] >= 0)
    sol_component_value(pm, 0, :bus, :deficit, ids(pm, 0, :bus), PowerModels.var(pm)[:deficit])
end
