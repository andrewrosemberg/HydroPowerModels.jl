...
function variable_volume(sp, data::Dict)
    @variable(sp, 0 <= reservoir[r = 1:data["hydro"]["nHyd"]] <= data["hydro"]["Hydrogenerators"][r]["max_volume"], SDDP.State, initial_value = data["hydro"]["Hydrogenerators"][r]["initial_volume"])
end

"""creates deficit variables"""
function variable_deficit(sp, data::Dict, pm::AbstractPowerModel)
    PowerModels.var(pm, 0, 1)[:deficit] = @variable(sp, deficit[i in keys(PowerModels.con(pm, 0, 1, :kcl_p))] >= 0)
    PowerModels.var(pm, 0, 1)[:deficit_q] = @variable(sp, deficit_q[i in keys(PowerModels.con(pm, 0, 1, :kcl_q))] >= 0)
end
